...
class CalendarView: NSView {
    ...
    private var fakeTodayFromInterfaceBuilder: SecularDate?

    private var today: SecularDate!
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        today = fakeTodayFromInterfaceBuilder ?? dataModel!.today
        ...
        let days = dataModel!.days
        let weekdayOfFirst = days[0].weekday.rawValue
        for (i, day) in days.enumerated() {
            let rect = rectArray[i + weekdayOfFirst]
            drawOneDay(i + 1, rect, day)
        }
    }
    ...
    private func drawOneDay(_ dayOfMonth: Int, _ rect: NSRect, _ day: CalendarDay) {
        assert(dayOfMonth == day.secularDate.day)
        NSBezierPath(rect: rect).stroke()
        if day.secularDate == today! {
        ...
        let hebrewDayString = "\(day.hebrewDate.day) \(day.hebrewMonthName)"
        let holidays = dataModel!.holidays(on: day)
    ...
    private func getBanner() -> String {
        let firstDay = dataModel!.days.first!
        let lastDay = dataModel!.days.last!
    ...
    override func prepareForInterfaceBuilder() {
        dataModel = CalendarMonth(year: 2027, month: 1)
        fakeTodayFromInterfaceBuilder = SecularDate(year: 2027, month: 1, day: 18)
    }
}
